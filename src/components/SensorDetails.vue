...
const typeLabels = {
  ...
  door: 'Contrôle d\'accès', smoke: 'Détecteur de fumée', hvac: 'Centrale de traitement d\'air',
    ...
    <Transition name="fade">
      <div v-if="sensor" class="sensor-detail__content" :key="sensor.id + sensor.status">
        ...
        <div v-if="hasProblem && sensor.reason" class="sensor-detail__reason" :class="'sensor-detail__reason--' + sensor.status">
          ...
          <button class="resolve-btn" :class="'resolve-btn--' + sensor.status" @click="emit('resolve-sensor', sensor.id)">
